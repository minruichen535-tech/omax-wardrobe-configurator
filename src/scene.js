...
function getWoodTopEdgeDiagnostic(placement, wall, worldBox, room) {
  ...
  if (wall?.id === "back" && isFirstBay) {
    expectedEdgeRole = "back-first-to-left-wall";
    expectedBoundaryWorld = -roomWidth / 2;
    actualEdgeWorld = toMm(worldBox.min.x);
    edgeAxis = "worldX";
    suggestedLocalDirection = actualEdgeWorld === expectedBoundaryWorld ? "none" : actualEdgeWorld > expectedBoundaryWorld ? "-localX" : "+localX";
  } else if (wall?.id === "back" && isLastBay) {
    expectedEdgeRole = "back-last-to-right-wall";
    expectedBoundaryWorld = roomWidth / 2;
    actualEdgeWorld = toMm(worldBox.max.x);
    edgeAxis = "worldX";
    suggestedLocalDirection = actualEdgeWorld === expectedBoundaryWorld ? "none" : actualEdgeWorld < expectedBoundaryWorld ? "+localX" : "-localX";
