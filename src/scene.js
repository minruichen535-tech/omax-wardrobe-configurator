...
const sceneRuntimeVersion = "woodtop-edge-diagnostics-20260603-01";
...
const componentTransformMap = {
  ...
  trouserRack: { rotation: [0, 0, 0], scaleAxis: "x", anchor: "bottomCenter", depthOffset: 0, heightOffset: 0, resizeMode: "centerInBay", offsetX: 0.015 }
};
...
async function addPlacement(group, placement, x, bayWidth, depth, config, design, series, report, debug, selectedId, wall = null) {
  ...
  if (model.name === "Missing Model") {
    ...
    model.position.x += transform.offsetX || 0;
  ...
  if (placement.componentType === "woodTop") {
    ...
    if (placement.autoGenerated && (edgeDiagnostic.suggestedLocalDirection === "-localX" || edgeDiagnostic.suggestedLocalDirection === "+localX")) {
    ...
    const isSideCornerWoodTop = placement.componentType === "woodTop"
      && placement.autoGenerated
    ...
    if (isSideCornerWoodTop) {
      ...
      const wallOffset = meters(Number(config.wallOffset) || 250);
      const targetWorldZ = -roomDepth / 2 + wallOffset + depth / 2;
      const currentEdgeWorldZ = worldBox.min.z;
      const deltaWorldZ = targetWorldZ - currentEdgeWorldZ;
      const localDeltaX = wall.id === "left" ? -deltaWorldZ : deltaWorldZ;
      model.position.x += localDeltaX;
      group.updateMatrixWorld(true);
      model.updateMatrixWorld(true);
      worldBox = new THREE.Box3().setFromObject(model);
      edgeDiagnostic = getWoodTopEdgeDiagnostic(placement, wall, worldBox, design.room);
