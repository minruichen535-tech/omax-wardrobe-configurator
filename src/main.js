...
import { WardrobeScene } from "./scene.js?v=wall-mounted-back-panel-seamless-20260718-01";
...
function getClientDrawerSingleProducts(data, isClientMode) {
  if (!isClientMode || data.series?.seriesId !== "japanese-closet") return [];
  return (data.products || [])
    .filter((product) => product.type === "drawerSingle" && product.clientOnly === true)
    .sort((a, b) => {
...
function getLibraryEntries({ cuttingRules, data, design, isClientMode }) {
  ...
  const clientDrawerEntries = getClientDrawerSingleProducts(data, isClientMode)
    .map((product) => ({
      key: `sku:${product.sku}`,
  ...
  const clientDrawerDoubleEntry = clientDrawerEntries.length
    ? [{
      key: "client:drawerDouble",
      componentType: "drawerDouble",
      isClientDrawerDouble: true,
      product: drawerDoubleProduct || clientDrawerEntries[0].product
    }]
    : [];
  return [...defaultEntries, ...clientDrawerEntries, ...clientDrawerDoubleEntry];
}
...
function ClientApp({ data, isClientMode = false }) {
  ...
  const clientDrawerSingleProducts = useMemo(
    () => getClientDrawerSingleProducts(data, isClientMode),
    [data, isClientMode]
  );
  ...
  const addPlacement = (wallId, bayIndex, componentType, options = {}) => {
    ...
    const defaultTopDrawerSku = clientDrawerSingleProducts[0]?.sku || "";
    const defaultBottomDrawerSku = clientDrawerSingleProducts[1]?.sku || defaultTopDrawerSku;
        ...
        h(StepBlock, { icon: Shirt, title: "组件库" },
          ...
          h("div", { className: "component-library" },
            getLibraryEntries({ cuttingRules, data, design, isClientMode }).map((entry) => {
              ...
              const icon = entry.productSku || entry.isClientDrawerDouble
              ...
              const entrySupportsClick = supportsLibraryClick || Boolean(entry.productSku) || entry.isClientDrawerDouble;
...
function PlacementEditor({
  ...
  return h("div", { className: "height-editor" },
    ...
    placement.componentType === "drawerDouble" && h("div", { className: "range-field" },
      ...
      h("select", {
        value: placement.topDrawerSku || clientDrawerSingleProducts[0]?.sku || "",
        onChange: (event) => updatePlacement(placement.id, { topDrawerSku: event.target.value })
      },
        clientDrawerSingleProducts.map((drawer) => h("option", {
          key: drawer.sku,
          value: drawer.sku
    ...
    placement.componentType === "drawerDouble" && h("div", { className: "range-field" },
      ...
      h("select", {
        value: placement.bottomDrawerSku || clientDrawerSingleProducts[1]?.sku || clientDrawerSingleProducts[0]?.sku || "",
