...
function bindProductFilters() {
  ...
  document.querySelector("#product-type-filter").addEventListener("change", (event) => {
...
function populateProductTypeFilter() {
  const select = document.querySelector("#product-type-filter");
  const types = [...new Set(getLibraryComponents().map((product) => product.type).filter(Boolean))]
    .sort();
  ...
  select.replaceChildren(new Option("全部类型", ""), ...options);
}
...
function handleSelectedItemInput(event) {
  ...
  const widthInput = event.target.closest("input[data-custom-width][data-placement-id]");
  if (!widthInput) return;
  const width = Number(widthInput.value);
  updatePlacement(widthInput.dataset.placementId, {
    customWidth: Number.isFinite(width) && width > 0 ? width : null
  });
}
...
function renderProductPicker() {
  ...
  container.classList.add("component-grid");
  const search = state.productSearch.toLowerCase();
  const cards = getLibraryComponents()
    .filter((product) => {
      const matchesSearch = !search
        || `${product.sku} ${product.nameCn} ${product.type}`.toLowerCase().includes(search);
      const matchesType = !state.productTypeFilter || product.type === state.productTypeFilter;
...
function getLibraryComponents() {
  return getConfigurableProducts().filter((product) => {
    ...
    if (/(post|立柱|调节脚|led|transformer|变压器)/i.test(searchable)) return false;
    return true;
  });
}
...
function createPlacementEditor(placement) {
  ...
  if (isStretchable(product)) {
    const widthLabel = document.createElement("label");
    widthLabel.className = "width-control";
    const label = document.createElement("span");
    const placementWidth = getPlacementDisplayWidth(placement);
    label.textContent = `宽度 mm（当前跨 ${placementWidth}）`;
    const input = document.createElement("input");
    input.type = "number";
    input.min = "1";
    input.step = "1";
    input.value = placement.customWidth ?? placementWidth ?? "";
    input.dataset.customWidth = "true";
    input.dataset.placementId = placement.id;
    widthLabel.append(label, input);
    editor.append(widthLabel);
  }
...
function renderParameterResults() {
  ...
  document.querySelector("#formula-bay-width-minus4").textContent =
    `bayWidthMinus4 = ${formatMeasurement(bayWidthMinus4)} mm`;
}
